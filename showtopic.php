<?php
	...
	if (mysqli_num_rows($get_topics_res) < 1) { 
		...
		while ($topic_info = mysqli_fetch_array($get_topics_res)) { 
			...
			//add to display 
			$display_block .= ' <tr> <td><a href="showtopic.php?topic_id='.$topic_id.'"> <strong>'.$topic_title.'</strong></a><br/> Created on '.$topic_create_time.' by '.$topic_owner.'</td> <td class="num_posts_col">'.$num_posts.'</td> </tr>'; 
		}
		//free results 
...
<html> 
   <head> 
      ...
      <style type="text/css"> table { border: 1px solid black; border-collapse: collapse; } th { border: 1px solid black; padding: 6px; font-weight: bold; background: #ccc; } td { border: 1px solid black; padding: 6px; } .num_posts_col { text-align: center; } </style> 
   </head> 
   <body>
